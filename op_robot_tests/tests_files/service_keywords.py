...
# def npv(annual_costs_reduction,
#         announcement_date, nbu_discount_rate, current_npv, 
#         percentage, days_per_year=365, npv_calculation_duration=20):
#     """ UA Net Present Value """
#     for i in range(1, 1000):
#         contract_duration_years = reversed(range(14))
#         contract_duration_days =  reversed(range(364))
#         yearly_payments_percentage = reversed(range(100,80))
...
#         npv = sum(discounted_income_by_periods)
#         if npv > current_npv:
#             break
#     return contract_duration_years, contract_duration_days, yearly_payments_percentage

def npv(annual_costs_reduction,
        announcement_date, nbu_discount_rate, current_npv, 
        days_per_year=365, npv_calculation_duration=20):
    """ UA Net Present Value """
    years_amount= list(reversed(range(14,0)))
    days_amount= list(reversed(range(364,1,10)))
    percentage_amount= list(reversed(range(100,80,2)))
    for years, days, percentage in itertools.izip(years_amount, days_amount, percentage_amount):
        if years >= 0:
            contract_duration_years = years
        else:
        ...
        if percentage >= 80:
            yearly_payments_percentage = percentage
        else:
            yearly_payments_percentage = 80
